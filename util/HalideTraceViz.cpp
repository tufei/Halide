...
void calc_2d_size(const std::vector<Range> &dims, const std::vector<Point> &strides, Range *x, Range *y,
    ...
    if (static_cast<unsigned int>(current_dimension) == dims.size()) {
...
struct Surface {
    ...
    void do_fill_realization(uint32_t *dst, uint32_t color,
        ...
        if (2 * current_dimension == p.dimensions) {
            ...
            const Point pt = static_cast<unsigned int>(current_dimension) < fi.config.strides.size() ? fi.config.strides.at(current_dimension) : Point{0, 0};
            x_off += pt.x * min;
            y_off += pt.y * min;
